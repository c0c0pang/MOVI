...
class ChatPageState extends State<ChatPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(widget.roomTitle, style: TextStyle(color: Colors.black)),
          centerTitle: true,
          elevation: 0,
        ),
        body: Container(
          child: Column(
            children: [
              Expanded(child: messageLog(context)),
              newMessage(),
            ],
          ),
        ));
  ...
  Widget messageLog(BuildContext context) {
    return StreamBuilder(
        stream: FirebaseFirestore.instance
            .collection('ChatRoom/rPDUIQvCg3PBVxuq3gR6/$term/$roomDoc/chat')
          ...
          if (snapshot.hasData) {
            return ListView.builder(
                ...
                itemBuilder: (context, int index) {
                  return chatBubble(docs[index]['id'], docs[index]['text'],
                      docs[index]['time'], docs[index]['name']);
                });
          } else {
            return Text('');
          }
                  ...
                  children: [
                    ...
                    Container(
                      child: Text(docs[index]['name'],
                          style: TextStyle(fontSize: 17, fontFamily: menuFont,fontWeight: FontWeight.bold)),
                    ), //닉네임
                    SizedBox(
                      height: 20,
                    ),
                    Container(
                      child: Text((docs[index]["Temperature"]).toStringAsFixed(2)),
                    ), //온도
                    SizedBox(
                      height: 20,
                    ),
                    ...
                    Container(
                      child: Column(
                        children: [
                          ...
                          Container(child: Text(docs[index]['area']))
                        ],
                      ),
                    ), //지역
                    SizedBox(
                      ...
                      child: Column(
                        children: [
                          ...
                          Container(
                            ...
                            children: [Text('#${docs[index]['genre']}')],
                          ))
                        ],
                      ),
                    ), //관심장르
  ...
  void upTempurture() {
    FirebaseFirestore.instance.collection('User').doc(selectid).update({
      'Temperature': selectUserTemp + 0.01,
    });
    print("온도 올라감");
  }
